...
async function readText(file: File): Promise<string> {
  ...
  try {
    ...
    // Older exports (e.g. some EndNote / Excel CSVs) use Windows-1252
...
export function ImportPage() {
  ...
  const analyse = async () => {
    ...
    if (/\.(pdf|docx?|xlsx?|zip|png|jpe?g)$/i.test(file.name)) {
      setError('Unsupported file type. Please upload a CSV, RIS, BibTeX (.bib) or PubMed (.nbib / .txt) export. Excel files: use “Save as CSV” first.');
      return;
    }
    setPhase('parsing');
    try {
      ...
      const resp = await parseInWorker({ text, fileName: file.name });
      if (!resp.ok) {
        setError(resp.error);
        setPhase('choose');
        return;
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6">
      ...
      <p className="mt-1 text-sm text-slate-600">
        ...
        Web of Science, Embase, CINAHL, IEEE Xplore, Google Scholar, Zotero, EndNote, Mendeley and others. Nothing is imported until you confirm the preview.
      </p>
        ...
        <Card className="mt-5 p-5">
          <div className="space-y-4">
            <Field id="file" label="Reference file">
              <input
                id="file" ref={fileRef} type="file" accept=".csv,.tsv,.txt,.ris,.bib,.bibtex,.nbib"
